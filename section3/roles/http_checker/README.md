...
завершения и валидирует exit code. Дополнительно вычитывает stdout
...
`exited`. Дефолты `30 × 2s = 60s`. Подкручивается, если httpstat.us
тормозит.
...
(запинован по digest), внутри инициализируется vfs storage driver, потом
зависимая роль `docker` ставит и стартует Docker Engine, после чего сама
роль тянет `imm174/http-checker:1.0.0`, гоняет один запрос и валидирует
exit code.
