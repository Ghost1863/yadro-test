...
Роль предполагает, что на хосте уже установлен Docker. В `site.yml` это
обеспечивается ролью `docker`, которая катается перед `http_checker`. При
самостоятельном использовании роли убедитесь, что Docker и Python SDK
(`python3-docker`) присутствуют на целевом хосте.

## Переменные

Все переменные роли типизированы в `meta/argument_specs.yml` список
кодов, числовые ретраи и булевы флаги валидируются до запуска задач.
...
сервису-зеркалу (по умолчанию `tools-httpstatus.pickup-services.com`,
оригинальный `httpstat.us` недоступен из РФ). Дефолт
`[100, 200, 301, 404, 500]`, по одному из каждой категории.
...
Задачи помечены тэгами: `image`/`pull`, `run` , `logs`/`verify` , `cleanup` . 
...
внутри инициализируется vfs storage driver, потом зависимая роль `docker`
ставит и стартует Docker Engine, после чего сама роль пуллит
`imm174/http-checker:1.0.0`, отправляет один запрос и валидирует exit
code.

В `test_sequence` намеренно нет шага `idempotence`: роль работает по
схеме `recreate: true`, второй прогон всегда
«changed»  это ожидаемое поведение, а не нарушение идемпотентности.
