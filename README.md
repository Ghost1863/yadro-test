...
В `section1/` лежит пакет `http_checker`, который отправояет запросы на сервис-зеркало
...
push в `master` (по путям `section1/` и `section2/`) GitHub Actions
запускает воркфлоу `.github/workflows/build-and-push.yml` из двух джоб:
`build` собирает образ, `push` логинится в Docker Hub через
`secrets.DOCKERHUB_TOKEN` и публикует тег.
